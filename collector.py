...
LOG_LINE_PATTERN = re.compile(
    r".*?\[(?P<timestamp>\d{1,2}/\d{1,2}/\d{4},\s*\d{1,2}:\d{2}:\d{2}\s*(?:AM|PM))\].*?\[eWeLink\]\[39m\s+\[(?P<device>[^\]]+)\]\s+current power\s+\[(?P<power>[^\]]+)\]\s+current voltage\s+\[(?P<voltage>[^\]]+)\]\s+current current\s+\[(?P<current>[^\]]+)\]"
)
